...
module "dynamodb_autoscaler" {
  source                       = "git::https://github.com/brockoffdev/terraform-aws-dynamodb-autoscaler.git?ref=tags/0.2.7-alt"
  enabled                      = "${var.enabled ? "${var.enable_autoscaler}" : "false"}"
  namespace                    = "${var.namespace}"
  stage                        = "${var.stage}"
  name                         = "${var.name}"
  delimiter                    = "${var.delimiter}"
  ...
  dynamodb_indexes             = ["${join("", null_resource.global_secondary_index_names.*.triggers.name)}"]
